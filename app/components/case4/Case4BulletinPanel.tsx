 ...
 * Case4BulletinPanel
 * 
 * Displays 3 IT bulletins with loading animation.
 * Status driven by currentTime prop (no internal timers).
 */

import React from 'react';
import { ITBulletin } from '@/app/lib/case4/demoCase4Data';

interface Case4BulletinPanelProps {
  bulletins: ITBulletin[];
  currentTime: number;
}

type BulletinStatus = 'queued' | 'loading' | 'loaded';

export default function Case4BulletinPanel({ bulletins, currentTime }: Case4BulletinPanelProps) {
  
  const getBulletinStatus = (bulletin: ITBulletin): BulletinStatus => {
    if (currentTime < 0) return 'queued';
    if (currentTime < bulletin.completion_time_ms) return 'loading';
    return 'loaded';
  };
